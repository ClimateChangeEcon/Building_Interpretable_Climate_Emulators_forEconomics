...
def simulate_dynamic_ignore(A_set, m0, T,
                            e=None,
                            c1=0.137,
                            c3=0.73,
                            c4=0.00689,
    ...
    for t in range(1, T):
        ...
        temprature_at[t] = temprature_at[t - 1] + delta_t * c1 * \
            (F_t - F2XCO2/T2XCO2*temprature_at[t-1] - c3*(delta_temp))
...
def load_results(root, test_name, conditions, model_set=[model_3sr, model_4pr], T_sim_set=[500]):
    ...
    for model_inx, model in enumerate(model_set):
        ...
        [m_benchmark, _] = data_load.pulse_fraction(
            test_type=benchmark_name, T=T_max, conditions=conditions)
        m_benchmark *= m0[0]
        ...
        for T in T_sim_set:
            results[model_name]['err_l1_' +
                                str(int(T))] = l1_err(m_sim[0, 0:T], m_benchmark[0:T]) / (T)
            results[model_name]['err_l2_' +
                                str(int(T))] = l2_err(m_sim[0, 0:T], m_benchmark[0:T]) / (T)
            results[model_name]['err_linf_' +
                                str(int(T))] = linf_err(m_sim[0, 0:T], m_benchmark[0:T])
            results[model_name]['err_sum_abs_' +
                                str(int(T))] = np.abs(np.sum(m_sim[0, 0:T] - m_benchmark[0:T]))
    ...
    for i, row in enumerate(table._rows):
        for j, val in enumerate(row):
            if isinstance(val, float):
                table._rows[i][j] = f"{val:.4f}"
            elif isinstance(val, (np.ndarray, list)):
                # NumPy array/matrix or Python list
                # ensure numeric for formatting
                arr = np.array(val, dtype=float)
                table._rows[i][j] = np.array2string(
                    arr,
