...
constants = {'Tstep': 1, # Years in one time step
            ...
            'pow2': 2.,
            #----------------Carbon mass---------------------------------
            'b12_': 0.0365264,   # Rate of carbon diffusion from atmosphere to upper ocean
            'b23_': 0.0032941,   # Rate of carbon diffusion from upper ocean to lower ocean
            'b14_': 0.0603739,  # Rate of carbon diffusion from atmosphere to land biosphere
            'MATeq': 0.589, # Equilibrium mass of carbon in the Atmosphere
            'MUOeq': 0.845, # Equilibrium mass of carbon in the upper Ocean
            'MLOeq': 37.375, #Equilibrium mass of carbon in the lower ocean
            'MLFeq': 0.470, #Equilibrium mass of carbon in the lower ocean
            #-------------------Temperature----------------------
            ...
            # Initial state
            'k0': 2.926,  # K0/(A0L0)
            'MAT0': 0.854,  # [1000 GtC] 2004
            'MUO0': 1.041,  # [1000 GtC] 2004
            'MLO0': 37.403, # [1000 GtC] 2004
            'MLF0': 0.637, # [1000 GtC] 2004
            'TAT0': 1.1,  # [oC relative to the preindustrial]
            'TOC0': 0.27, # [oC relative to the preindustrial]
            'tau0': 0.,  # Initial time period
            }

#print("number of constants:", len(constants))
...
MATx_state = []
MATx_state.append({'name':'MATx','init':{'distribution':'truncated_normal','kwargs': {'mean':0.854,'stddev':0.}}})

MUOx_state = []
MUOx_state.append({'name':'MUOx','init':{'distribution':'truncated_normal','kwargs': {'mean':1.041,'stddev':0.}}})

MLOx_state = []
MLOx_state.append({'name':'MLOx','init':{'distribution':'truncated_normal','kwargs': {'mean':37.403,'stddev':0.}}})

MLFx_state = []
MLFx_state.append({'name':'MLFx','init':{'distribution':'truncated_normal','kwargs': {'mean':0.637,'stddev':0.}}})
...
end_state = kx_state + MATx_state + MUOx_state + MLOx_state + MLFx_state + TATx_state + TOCx_state
...
#print(" =========================================================")
#print("number of EWMA normalizing states",  len(hh_ewma_S_n_state))
#print(" =========================================================")
...
nuAT_haty_policy = []
nuAT_haty_policy.append({'name':'nuAT_haty','activation': 'tf.keras.activations.softplus'})

nuUO_haty_policy = []
nuUO_haty_policy.append({'name':'nuUO_haty','activation': 'tf.keras.activations.linear'})

nuLO_haty_policy = []
nuLO_haty_policy.append({'name':'nuLO_haty','activation': 'tf.keras.activations.linear'})

nuLF_haty_policy = []
nuLF_haty_policy.append({'name':'nuLF_haty','activation': 'tf.keras.activations.linear'})

etaAT_haty_policy = []
etaAT_haty_policy.append({'name':'etaAT_haty','activation': 'tf.keras.activations.linear'})

etaOC_haty_policy = []
etaOC_haty_policy.append({'name':'etaOC_haty','activation': 'tf.keras.activations.linear'})
...
policies = kplusy_policy + lambd_haty_policy +\
            nuAT_haty_policy + nuUO_haty_policy + \
            nuLO_haty_policy + nuLF_haty_policy + etaAT_haty_policy + etaOC_haty_policy #+ kappa_policy
...
definitions = tau2t_policy + tau2tauplus_policy + tfp_policy + \
                ...
                Eind_policy + scc_policy + carbontax_policy + \
                Dam_policy + Emissions_policy +\
                MATplus_policy + MUOplus_policy + MLOplus_policy +\
                MLFplus_policy +TATplus_policy + TOCplus_policy



# print("number of definitions", len(definitions)) # 2N
