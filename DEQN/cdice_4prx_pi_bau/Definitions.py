...
b12_, b23_, b14_, MATeq, MUOeq, MLOeq = Parameters.b12_, Parameters.b23_, Parameters.b14_, \
    Parameters.MATeq, Parameters.MUOeq, Parameters.MLOeq
# Temperature exchange
c1_, c3_, c4_  = Parameters.c1_, Parameters.c3_, \
    Parameters.c4_
f2xco2, t2xco2, kappa =  Parameters.f2xco2, Parameters.t2xco2, Parameters.kappa
...
def b11(state, policy_state):
    """ Mass of carbon transmission"""
    return 1 - Tstep * b12_ - Tstep * b14_

def b21(state, policy_state):
    """ Mass of carbon transmission"""
    return MATeq/MUOeq * b12_* Tstep

def b41(state, policy_state):
    """ Mass of carbon transmission"""
    _MLFeq = State.MLFeqx(state)
    return MATeq/_MLFeq * b14_* Tstep
...
def b22(state, policy_state):
    """ Mass of carbon transmission"""
    return 1 - Tstep * MATeq/MUOeq * b12_ - Tstep * b23_
...
def b33(state, policy_state):
    """ Mass of carbon transmission"""
    return 1 - Tstep * MUOeq/MLOeq * b23_



def b14(state, policy_state):
    """ Mass of carbon transmission"""
    return Tstep * b14_

def b44(state, policy_state):
    """ Mass of carbon transmission"""
    _MLFeq = State.MLFeqx(state)
    return 1 - Tstep * b14_ * MATeq/_MLFeq
...
def carbontax(state, policy_state):
    ...
    _carbontax = _theta1 * theta2  /_sigma
    return _carbontax
...
def scc(state, policy_state):
    ...
    _tfp = tfp(state, policy_state)
    _lab = lab(state, policy_state)
    _b11 = b11(state, policy_state)
    _b12 = b12(state, policy_state)
    _b14 = b14(state, policy_state)
    _nuUO_haty = PolicyState.nuUO_haty(policy_state)
    _nuLF_haty = PolicyState.nuLF_haty(policy_state)
    ...
    _dvdMAT =  ((-_nuAT_haty) * _b11 + _nuUO_haty * _b12 + _nuLF_haty * _b14
...
def MATplus(state, policy_state):
    ...
    _MLFx = State.MLFx(state)
    _b11 = b11(state, policy_state)
    _b21 = b21(state, policy_state)
    _b41 = b41(state, policy_state)
    _MATplus = _b11 * _MATx + _b21 * _MUOx + _b41 * _MLFx \
...
def MUOplus(state, policy_state):
    ...
    _b22 = b22(state, policy_state)
    _b12 = b12(state, policy_state)
    _MUOplus = _b12 * _MATx + _b22 * _MUOx + _b32 * _MLOx
    return _MUOplus
...
def MLOplus(state, policy_state):
    ...
    _b33 = b33(state, policy_state)
    _MLOplus = _b23 * _MUOx + _b33 * _MLOx
    return _MLOplus

def MLFplus(state, policy_state):
    """ Carbon mass in the land forest """
    _MATx = State.MATx(state)
    _MLFx = State.MLFx(state)
    _b14 = b14(state, policy_state)
    _b44 = b44(state, policy_state)
    _MLFplus = _b14 * _MATx + _b44 * _MLFx
    return _MLFplus

def MLFeqplus(state, policy_state):
    """ Carbon mass in the land forest """
    _MLFeqx = State.MLFeqx(state)
    _Eland = Eland(state, policy_state)
    _MLFeqplus = _MLFeqx - _Eland
    return _MLFeqplus
...
def TATplus(state, policy_state):
    ...
    _MATx = State.MATx(state)
    _c1 = c1(state, policy_state)
    _c3 = c3(state, policy_state)
    _f2xco2 = Parameters.f2xco2
    _t2xco2 = Parameters.t2xco2
    _TATplus = _TATx + Tstep * _c1 * (_f2xco2 * kappa * tf.math.log(_MATx / MATbase) / tf.math.log(2.)) - \
                Tstep * _c1 * (_f2xco2 / _t2xco2) * _TATx - Tstep * _c1 * _c3 * (_TATx - _TOCx)
    return _TATplus
...
def TOCplus(state, policy_state):
    ...
    _TOCplus = _TOCx + _c4 * (_TATx - _TOCx)
    return _TOCplus
