"""
Filename: post_process_4sr_eq.py
Description:
...
econ_defs = ['con', 'Omega', 'Theta', 'ygross', 'ynet', 'inv', 'Eind',
             'carbontax', 'Abatement', 'Dam', 'Emissions']
...
starting_state = tf.reshape(tf.constant([
    ...
    Parameters.MLF0, Parameters.MLFeq0, Parameters.TAT0, Parameters.TOC0,
...
for sidx, state in enumerate(Parameters.states):
    ...
    elif state in ['MATx', 'MUOx', 'MLOx', 'MLFx', 'MLFeqx']:
        # Rescale to GtC
