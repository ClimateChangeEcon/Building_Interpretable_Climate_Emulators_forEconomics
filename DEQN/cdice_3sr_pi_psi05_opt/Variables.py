...
constants = {'Tstep': 1, # Years in one time step
            ...
            # Utility function
            'rho': 0.004, #0.004,  # Discount factor in continuous time
            'psi': 0.5, #0.5,  # CRRA: intertemporal elasticity of substitution/risk aversion (Cai and Lontzek, 2019)
            # Production function
            ...
            'pow2': 2.,
            #----------------Carbon mass---------------------------------
            'b12_': 0.0769,   # Rate of carbon diffusion from atmosphere to upper ocean
            'b23_': 0.0109,   # Rate of carbon diffusion from upper ocean to lower ocean
            'MATeq': 0.589, # Equilibrium mass of carbon in the Atmosphere
            'MUOeq': 0.752, # Equilibrium mass of carbon in the upper Ocean
            'MLOeq': 1.289, #Equilibrium mass of carbon in the lower ocean
            #-------------------Temperature----------------------
            ...
            # Initial state
            'k0': 2.926,  # K0/(A0L0)
            'MAT0': 0.851,  # [1000 GtC] 2004
            'MUO0': 0.983,  # [1000 GtC] 2004
            'MLO0': 1.377, # [1000 GtC] 2004
...
MATx_state = []
MATx_state.append({'name':'MATx','init':{'distribution':'truncated_normal','kwargs': {'mean':0.851,'stddev':0.}}})

MUOx_state = []
MUOx_state.append({'name':'MUOx','init':{'distribution':'truncated_normal','kwargs': {'mean':0.983,'stddev':0.}}})

MLOx_state = []
MLOx_state.append({'name':'MLOx','init':{'distribution':'truncated_normal','kwargs': {'mean':1.377,'stddev':0.}}})
...
end_state = kx_state + MATx_state + MUOx_state + MLOx_state + TATx_state + TOCx_state
...
policies = kplusy_policy + lambd_haty_policy +\
            ...
            nuLO_haty_policy + etaAT_haty_policy + etaOC_haty_policy #+ kappa_policy
