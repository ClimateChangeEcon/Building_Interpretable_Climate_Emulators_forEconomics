...
def equations(state, policy_state):
    ...
    nuLF_haty = PolicyState.nuLF_haty(policy_state)
    ...
    # FOC wrt. MLOplus
    # ----------------------------------------------------------------------- #
    loss_dict['foc_MLFplus'] = nuLF_haty - beta_hat * E_t(
        lambda s, ps:
        (-PolicyState.nuAT_haty(ps)) * b41
        + PolicyState.nuLF_haty(ps) * b44
    )
