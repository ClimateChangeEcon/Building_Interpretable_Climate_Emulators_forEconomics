...
def uvic_pulse(T_start: int = 0, T_end: int = 500, pulse='100GtC', folder_name: str = 'data/uvic_pulse/'):


    #Time	100GtC	200GtC	500GtC	-100GtC	-200GtC	-500GtC	

    header = np.loadtxt(folder_name + '/atm.txt',skiprows=0,max_rows=1,dtype=str)

    col_inx = np.array(np.where(header == pulse)[0])

    data={}

    data['atm']   = np.loadtxt(folder_name + '/atm.txt',  skiprows=1,dtype=float,usecols=col_inx)[T_start:T_end]
    data['ocean'] = np.loadtxt(folder_name + '/ocean.txt',skiprows=1,dtype=float,usecols=col_inx)[T_start:T_end]
    data['land']  = np.loadtxt(folder_name + '/land.txt', skiprows=1,dtype=float,usecols=col_inx)[T_start:T_end]
    data['temp']  = np.loadtxt(folder_name + '/temp.txt', skiprows=1,dtype=float,usecols=col_inx)[T_start:T_end]
